...
const SubModelItem = koishi_1.Schema.object({
    ...
    // 副模型独立配置 API 类型和地址
    apiType: koishi_1.Schema.union([
        koishi_1.Schema.const('auto').description('跟随主模型设置'),
        koishi_1.Schema.const('chat').description('强制聊天补全模式 (/v1/chat/completions)'),
        koishi_1.Schema.const('images').description('强制图像生成模式 (/v1/images/generations)'),
...
exports.ModelConfig = koishi_1.Schema.object({
    ...
    // 主模型独立配置 API 类型和地址
    apiType: koishi_1.Schema.union([
        koishi_1.Schema.const('auto').description('自动检测（先尝试标准图像API，失败则回退到聊天API）'),
        koishi_1.Schema.const('chat').description('强制聊天补全模式 (/v1/chat/completions)'),
        koishi_1.Schema.const('images').description('强制图像生成模式 (/v1/images/generations)'),
...
async function apply(ctx, cfg) {
     ...
     * @param subModel 副模型配置（可选）
     */
    function getEffectiveApiType(subModel) {
        if (subModel) {
     ...
     * 尝试用标准图像生成 API 调用
     */
    async function tryImagesApi(session, prompt, imageUrl, api, model, subModel) {
        ...
        try {
            ...
            if (status === 404 || status === 400 || status === 405) {
                if (debug)
                    logger.info(`图像生成API不支持 (${status})，将回退到聊天API`);
     ...
     */
    async function tryChatApi(session, prompt, imageUrl, api, model) {
            ...
            if (apiType === 'images') {
                ...
                if (imagesRes.success) {
                    ...
                    if (chatRes.success) {
                        ...
                        usedApiType = 'chat';
                        if (debug)
                            logger.info('自动模式：图像API不支持，回退到聊天API成功');
                    }
                    else {
