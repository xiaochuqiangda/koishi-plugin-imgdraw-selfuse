...
async function apply(ctx, cfg) {
    ...
    function ensureValidSize(width, height) {
    ...
    async function getImageDimensionsFromUrl(url) {
        ...
        try {
            ...
            if (debug)
                logger.warn('获取图片尺寸失败:', e.message);
            return null;
        }
    }
    // 无关键词时：最短边 1024，保持原比例，snap 到 16 的倍数
    function computeDynamicSize(refWidth, refHeight) {
    ...
    function compute4KSize(refWidth, refHeight) {
    ...
    async function resolveGenerationParams(prompt, referenceImageUrl) {
        if (!prompt)
            return { cleanedPrompt: prompt || '', size: '1024x1024' };
        let cleaned = prompt;
        ...
        if (customMatch) {
            ...
            if (debug)
                logger.info(`检测到自定义分辨率，修正后 size: ${customSize}`);
        }
        cleaned = cleaned.replace(/\s+/g, ' ').trim();
        ...
        if (referenceImageUrl) {
            ...
            if (debug && dims)
                logger.info(`参考图尺寸: ${dims.width}x${dims.height}`);
        }
        let size;
        if (customSize) {
            size = customSize;
            ...
            if (debug)
                logger.info(`横屏${has4K ? ' 4K' : ''}，size: ${size}`);
        }
        else if (isPortrait) {
            size = has4K ? ensureValidSize(2160, 3840) : '1024x1792';
            if (debug)
                logger.info(`竖屏${has4K ? ' 4K' : ''}，size: ${size}`);
        }
        else if (isSquare) {
            size = has4K ? ensureValidSize(2880, 2880) : '1024x1024';
            if (debug)
                logger.info(`方图${has4K ? ' 4K' : ''}，size: ${size}`);
        }
        else if (has4K) {
            ...
            if (dims) {
                ...
                if (debug)
                    logger.info(`4K + 参考图 ${dims.width}x${dims.height}，size: ${size}`);
            }
            else {
                size = ensureValidSize(3840, 2160);
                if (debug)
                    logger.info(`4K 无参考图，默认 size: ${size}`);
            }
        }
        else {
            ...
            if (dims) {
                ...
                if (debug)
                    logger.info(`动态 size (参考图 ${dims.width}x${dims.height}): ${size}`);
            }
            else {
                size = '1024x1024';
            }
