...
export const BaseConfig = Schema.object({
  ...
  img2imgPrompt: Schema.string().default('请根据以下指令对提供的图片进行编辑或重绘，不要询问，直接输出结果。你可以使用联网功能获取最新的数据或信息。\n指令：{prompt}').description('图生图提示词模板'),
...
export async function apply(ctx: Context, cfg: any) {
  ...
  function detectImageMimeType(buffer: Buffer): string {
    if (buffer.length < 2) return 'image/png'
  ...
  async function sendImage(session: any, imgUrl: string): Promise<void> {
    ...
    if (trimmed.startsWith('data:image/')) {
      ...
      if (debug) logger.info('下载 URL 图片:', trimmed.slice(0, 100))
      try {
        ...
        if (debug) logger.info(`下载完成，格式: ${mimeType}, base64 长度: ${base64.length}`)
        await safeSend(session, h('img', { src: base64 }))
      } catch (e) {
        logger.error('下载图片失败:', (e as any).message)
  ...
  function isImagesApi(url: string): boolean {
  ...
  async function callApi(
    ...
    model: string,
    subModel?: any
  ): Promise<{ success: boolean; result?: any; error?: string; rawError?: any }> {
    ...
    if (isImagesApi(url)) {
      ...
      if (imageUrl) {
        if (Array.isArray(imageUrl)) {
          ...
          if (debug) logger.info(`图生图添加参考图片数组: ${imageUrl.length} 张`)
        } else {
      ...
      if (debug) {
        ...
        if (logBody.images) logBody.images = `[${logBody.images.length}张图片数据]`
  ...
  async function generate(session: any, prompt: string, imageUrl?: string, subModel?: any, mode?: 'txt2img' | 'img2img'): Promise<void> {
    ...
    try {
      recordApiCall()
      let processedImageUrl: string | undefined = imageUrl
